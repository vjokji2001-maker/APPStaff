...
class _IpdDashboardPageState extends State<IpdDashboardPage> {
  ...
  void _refreshDashboardData() {
    setState(() {
      String apiWardId = "0";
      _dashboardDataFuture = ipdService
          .fetchDashboardData(wardId: apiWardId)
          .then((data) {
  ...
  Future<void> _fetchPractitionerList() async {
    try {
      ...
      debugPrint('Error loading practitioner list: $e');
    }
  }
    ...
    switch (_selectedFilterCategory) {
      case 'Available':
        tempPatients = tempPatients
            .where((p) => p.active == 0 && p.isUnderMaintenance == 0)
            .toList();
        break;
      case 'To be Discharged':
        tempPatients = tempPatients
            .where(
              (p) => p.dischargeStatus != '0' && p.dischargeStatus != '0.0',
            )
            .toList();
        break;
      case 'Excess Amount':
      ...
      default:
        tempPatients = tempPatients
            .where(
              (p) =>
                  p.active == 1 || (p.active == 0 && p.isUnderMaintenance == 0),
            )
            .toList();
        break;
    }
  ...
  void _calculateBedStats() {
    ...
    for (var p in data) {
      ...
      if (p.active == 1) {
        inhouseLen++;
        if (p.party.toLowerCase().contains('corporate') &&
            p.isUnderMaintenance == 0) {
          pTpLen++;
        } else if (p.active == 1 &&
            (p.isMlc == '0' || p.isMlc == '0.0') &&
        ...
        if ((p.dischargeStatus != '0' && p.dischargeStatus != '0.0') &&
            p.isUnderMaintenance == 0) {
          dischargeLen++;
        }
      }
          ...
          FutureBuilder<IpdDashboardData>(
            ...
            builder: (context, snapshot) {
              ...
              if (_allPatients.isEmpty &&
                  snapshot.hasData &&
                  snapshot.data!.patients.isNotEmpty) {
                            ...
                            child: GridView.builder(
                              ...
                              itemBuilder: (context, index) {
                                return PatientGridCardCompact(
                                  ...
                                  onCardTap: () => _showPatientQuickActionSheet(
            ...
            children: [
              Text(
                ...
                style: GoogleFonts.poppins(
                  ...
                  color: isSelected
                      ? color
                      : (label == "Inhouse" ? Colors.brown : color),
                  fontSize: 14,
                  height: 1,
                ),
              ),
                      ...
                      Expanded(
                        child: ElevatedButton(
                          ...
                          onPressed: () => setModalState(() {
                            tempSelectedWard = 'All Ward';
                            tempSelectedStatus = 'Active';
                            tempSelectedCategory = 'Format1';
  ...
  void _showPatientQuickActionSheet(Patient patient) {
    if (patient.active == 0) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("Bed ${patient.bedname} is Available")),
      );
      return;
    }
      ...
      },
      {
        ...
        'onTap': () {
          ...
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (_) => ReqInvestigationPage(
                patientName: patient.patientname,
                patient: patient,
              ),
            ),
      ...
      },
      {
        ...
        'onTap': () {
          ...
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (_) => TreatmentRecordPage(patient: patient),
            ),
          ).then((_) => _refreshDashboardData());
        },
      },
      ...
      },
      {
        ...
        'onTap': () {
          ...
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (_) => UploadDocScreen(patient: patient),
            ),
          ).then((_) => _refreshDashboardData());
        },
      },
        ...
        child: Column(
          ...
          children: [
            ...
            ListTile(
              ...
              subtitle: Text(
                "${patient.ward} | Bed: ${patient.bedname}\nIPD: ${patient.ipdNo}",
              ...
              trailing: Column(
                ...
                children: [
                  Text(
                    ...
                    style: TextStyle(
                      fontWeight: FontWeight.bold,
                      color: patient.patientBalance > 0
                          ? Colors.red
                          : Colors.green,
                    ),
                  ),
...
class PatientGridCardCompact extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    else if (dischargeStr == '1' || dischargeStr == 'true')
      statusColor = Colors.yellow;
    else if (mlcStr == '1' || mlcStr == 'true')
      statusColor = Colors.redAccent;
    else if (hasExcess)
      statusColor = Colors.orangeAccent;
    else if (party.contains('corporate'))
      statusColor = Colors.pink.shade200;
    else if (party.contains('third party'))
      statusColor = Colors.lightGreen.shade400;
    else if (party == 'self')
      statusColor = Colors.blueGrey.shade500;
    else
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Stack(
          children: [
            Container(color: statusColor.withValues(alpha: 0.08)),
            Positioned(
              left: 0,
              top: 0,
              bottom: 0,
              ...
              child: isAvailable
                  ? Center(
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            (patient.bedname as dynamic)?.toString() ?? 'N/A',
                            style: GoogleFonts.poppins(
                              fontWeight: FontWeight.bold,
                              fontSize: 13,
                  ...
                  : Column(
                      ...
                      children: [
                        Row(
                          children: [
                            CircleAvatar(
                              ...
                              child: Text(
                                ((patient.patientname as dynamic)?.toString() ??
                                            '')
                                        .isNotEmpty
                                    ? (patient.patientname as dynamic)
                                          .toString()[0]
                                    : "?",
                                style: TextStyle(
                                  fontSize: 10,
                            ...
                            Expanded(
                              child: Text(
                                (patient.bedname as dynamic)?.toString() ?? '',
                                style: GoogleFonts.poppins(
                                  fontWeight: FontWeight.bold,
                                  fontSize: 13,
                                  color: Colors.black87,
                        ...
                        Text(
                          (patient.ward as dynamic)?.toString() ?? '',
                          maxLines: 1,
                          overflow: TextOverflow.ellipsis,
                          style: TextStyle(
                            fontSize: 9,
                        ...
                        Text(
                          "IPD: ${(patient.ipdNo as dynamic)?.toString() ?? ''}",
                          style: TextStyle(
                            fontSize: 9,
                            color: Colors.grey[600],
                          ),
                ...
                child: BlinkingNotesIcon(
                  ...
                  onNotesTap: () async {
                    await showDialog(
                      context: context,
                      builder: (context) =>
                          DayToDayNotesDialog(patient: patient),
                    );
                    if (onRefresh != null) {
                      onRefresh!();
                    }
