...
class _AddChargesScreenState extends State<AddChargesScreen> {
  ...
  void _initializeLocation() {
    ...
    } else if (widget.patient.bedname != null &&
        widget.patient.bedname!.isNotEmpty) {
  ...
  Future<void> _fetchChargeTypes() async {
    ...
    try {
      ...
      if (mounted)
        setState(() {
          _chargeTypeList = chargeTypes;
          _isLoadingChargeTypes = false;
        });
    } catch (e) {
      if (mounted)
        setState(() {
          _chargeTypeError = e.toString();
          _isLoadingChargeTypes = false;
        });
    }
  }
  ...
  Future<void> _fetchChargeNames(String? chargeTypeId) async {
    ...
    try {
      ...
      if (response != null &&
        ...
        final chargeList = PackageService.extractChargeListFromMasterDetail(
          safeResponse,
        );
          ...
          setState(() {
            ...
            if (_chargeNameList.isNotEmpty) {
              ...
              if (firstCharge is Map) {
                _onChargeNameSelected(
                  firstCharge['id']?.toString(),
                  Map<String, dynamic>.from(firstCharge),
                );
              }
            }
          });
        ...
        if (mounted)
          setState(() {
            _chargeNameList = [];
            _isLoadingChargeNames = false;
            _chargeNameError = 'Failed to load';
          });
      }
    } catch (e) {
      if (mounted)
        setState(() {
          _chargeNameError = e.toString();
          _isLoadingChargeNames = false;
        });
    }
  }
  ...
  void _onChargeNameSelected(String? id, Map<String, dynamic> selectedCharge) {
    setState(() {
      ...
      if (selectedCharge['chargeDiscount'] != null) {
        double discount =
            double.tryParse(selectedCharge['chargeDiscount'].toString()) ?? 0.0;
        if (discount > 0) {
  ...
  Future<void> _fetchSpecializations() async {
    ...
    try {
      final specializations = await IpdService().fetchSpecializationList(
        branchId: "1",
      );
      if (mounted)
        setState(() {
      ...
      if (mounted)
        setState(() {
          _specializationError = e.toString();
          _isLoadingSpecializations = false;
        });
    }
  }
  ...
  void _calculateTotal() {
    ...
    if (_selectedDiscountType == 'percentage' && discountAmount > 0) {
      ...
      total -= discountValue;
    } else if ((_selectedDiscountType == 'amount' ||
            _selectedDiscountType == 'fixed') &&
        discountAmount > 0) {
  ...
  void _addChargeItem() {
    if (_selectedChargeType == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Please select charge type")),
      );
      return;
    }
    if (_selectedChargeId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Please select charge name")),
      );
      return;
    }
    ...
    for (var chargeType in _chargeTypeList) {
      if (chargeType is Map &&
          (chargeType['id'].toString() == _selectedChargeType ||
              chargeType['name']?.toString() == _selectedChargeType)) {
    ...
    final newCharge = {
      ...
      'discountType': _selectedDiscountType == 'percentage'
          ? '%'
          : (_selectedDiscountType == 'amount' ? 'Amount' : 'Fixed'),
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text("Item Added: $_selectedChargeDisplayName"),
  ...
  Future<void> _createChargeInvoice() async {
    if (_addedCharges.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Please add at least one charge")),
      );
      return;
    }
    ...
    try {
      ...
      for (var charge in _addedCharges) {
        ...
        final discountAmount =
            double.tryParse(charge['discountAmount'] ?? '0') ?? 0.0;
        final totalAmount = unitPrice * quantity - discountAmount;

        childListDTO.add(
          PackageService.createChildListItem(
                    ...
                    Expanded(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            widget.patient.practitionername,
                            style: GoogleFonts.poppins(
                              color: Colors.white,
                              fontSize: 12,
                          ...
                          children: [
                            Expanded(
                              child: _buildSelectableField(
                                ...
                                items: widget.practitionerList
                                    .map(
                                      (p) => p['practitionername'].toString(),
                                    )
                                    .toSet()
                                    .toList(),
                                onSelect: (v) =>
                                    setState(() => _selectedPractitioner = v),
                                isSearchable: true,
                              ),
                            ),
                            const SizedBox(width: 8),
                            ...
                            Expanded(
                              ...
                              child: _buildSelectableField(
                                ...
                                items: _chargeTypeList
                                    ...
                                    .toList(),
                                onSelect: (name) {
                                  final type = _chargeTypeList.firstWhere(
                                    (e) => e['name'] == name,
                                  );
                        ...
                        _isLoadingChargeNames
                            ...
                            : _buildSelectableField(
                                ...
                                items: _chargeNameList
                                    ...
                                    .toList(),
                                onSelect: (name) {
                                  final charge = _chargeNameList.firstWhere(
                                    (e) => e['name'] == name,
                                  );
                                  if (charge is Map) {
                                    _onChargeNameSelected(
                          ...
                          children: [
                            ...
                            Expanded(
                              child: Column(
                                ...
                                children: [
                                  Text(
                                    "${_addedCharges.length} Items Pending",
                                    style: GoogleFonts.poppins(
                                      fontWeight: FontWeight.w600,
                                      fontSize: 13,
                                    ),
          ...
          child: Row(
            children: [
              ...
              Expanded(
                flex: 2,
                child: ElevatedButton(
                  onPressed: _addedCharges.isNotEmpty
          ...
          child: Row(
            ...
            children: [
              Icon(
                ...
                color: isSelected ? darkBlue : Colors.grey,
                size: 16,
              ),
          ...
          child: Container(
            ...
            child: Row(
              children: [
                Icon(
                  icon,
                  color: value != null ? darkBlue : Colors.grey[500],
                  size: 16,
                ),
                const SizedBox(width: 8),
                Expanded(
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                ...
                                if (double.tryParse(item['discountAmount'])! >
                                    0)
                                  Text(
