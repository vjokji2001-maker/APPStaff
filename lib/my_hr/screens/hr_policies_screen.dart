...
class _HRPoliciesScreenState extends State<HRPoliciesScreen> {
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          HRGradientHeader(
            title: 'HR Policies',
            subtitle: 'Company policies & SOPs',
          ),
          // Search
          Padding(
          ...
          Expanded(
            child: _filtered.isEmpty
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (_, i) =>
                        _buildPolicyCard(_filtered[i], isDark),
                  ),
          ),
        ],
      ),
        ...
        children: [
                    ...
                    Row(
                      children: [
                        HRStatusBadge(
                          label: p.category,
                          color: catColor,
                          bgColor: catColor.withValues(alpha: 0.1),
                        ),
                        const SizedBox(width: 8),
                        HRStatusBadge(
                          label: p.version,
                          color: HRTheme.textSecondary,
                          bgColor: HRTheme.divider,
                        ),
                      ],
                    ),
                  ],
          ...
          GestureDetector(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                color: HRTheme.policies.withValues(alpha: 0.06),
                borderRadius: BorderRadius.circular(HRTheme.radiusSM),
                border: Border.all(
                  color: HRTheme.policies.withValues(alpha: 0.2),
                ),
              ),
              child: Row(
  ...
  Color _categoryColor(String cat) {
    switch (cat) {
      case 'Attendance':
        return HRTheme.attendance;
      case 'Leave':
        return HRTheme.leave;
      case 'Payroll':
        return HRTheme.payroll;
      case 'HR':
