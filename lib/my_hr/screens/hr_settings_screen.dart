...
class _HRSettingsScreenState extends State<HRSettingsScreen> {
        ...
        children: [
          Row(
            children: [
              CircleAvatar(
                radius: 28,
                backgroundColor: HRTheme.primaryDark.withValues(alpha: 0.12),
                child: Text(
                  _empInitials,
                  style: GoogleFonts.poppins(
                    fontSize: 18,
  ...
  Future<void> _saveProfile() async {
    ...
    await prefs.setString('mobileNo', phone);

    final cachedJson = prefs.getString('user_data_json');
    if (cachedJson != null && cachedJson.isNotEmpty) {
      try {
        ...
        if (data is Map) {
          data['fullName'] = name;
          data['mobileNo'] = phone;
          await prefs.setString('user_data_json', jsonEncode(cached));
        }
      } catch (_) {}
    }
    ...
    setState(() {
      _empName = name;
      _empInitials = name[0].toUpperCase();
    });
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
    ...
    return HRCard(
      ...
      child: Column(
        children: [
          _notifTile(
            'Salary & Payroll',
            'Get notified when salary is credited',
            _notifSalary,
            (v) => setState(() => _notifSalary = v),
            Icons.payments_outlined,
            HRTheme.payroll,
          ),
          _notifTile(
            'Leave Updates',
            'Approval status for leave requests',
            _notifLeave,
            (v) => setState(() => _notifLeave = v),
          ...
          _notifTile(
            'Reminders',
            'Training, document, shift reminders',
            _notifReminder,
          ...
          _notifTile(
            'Alerts',
            'Critical HR alerts and notifications',
            _notifAlert,
            (v) => setState(() => _notifAlert = v),
            Icons.error_outline_rounded,
            HRTheme.error,
          ),
          _notifTile(
            'Circulars',
            'Policy circulars and documents',
            _notifCircular,
            (v) => setState(() => _notifCircular = v),
        ...
        children: [
          ListTile(
            ...
            leading: Container(
              padding: const EdgeInsets.all(7),
              decoration: BoxDecoration(
                color: HRTheme.primaryDark.withValues(alpha: 0.1),
                borderRadius: BorderRadius.circular(HRTheme.radiusXS),
              ),
              child: Icon(
                _isDarkMode
      ...
      child: DropdownButtonFormField<String>(
        ...
        items: _languages
            .map(
              (l) => DropdownMenuItem(
                value: l,
                child: Text(l, style: GoogleFonts.poppins(fontSize: 13)),
              ),
            )
            .toList(),
        ...
        children: [
          ...
          ListTile(
            ...
            leading: Container(
              padding: const EdgeInsets.all(7),
              decoration: BoxDecoration(
                color: HRTheme.error.withValues(alpha: 0.1),
                borderRadius: BorderRadius.circular(HRTheme.radiusXS),
              ),
              child: const Icon(
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          TextButton(
            ...
            child: Text(
              'Cancel',
              style: GoogleFonts.poppins(color: HRTheme.textSecondary),
            ),
          ),
          ElevatedButton(
                ...
                TextField(
                  ...
                  decoration: InputDecoration(
                    ...
                    suffixIcon: IconButton(
                      ...
                      onPressed: () =>
                          setSS(() => _hideCurrent = !_hideCurrent),
                    ),
                    border: OutlineInputBorder(
                      borderRadius: BorderRadius.circular(HRTheme.radiusSM),
                    ),
