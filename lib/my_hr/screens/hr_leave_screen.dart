...
class _HRLeaveScreenState extends State<HRLeaveScreen> {
      ...
      final response = await http.post(
        ...
        body: jsonEncode({
          ...
          "messages": [
            {
              "role": "system",
              "content": "You are a JSON API. Return ONLY a valid JSON array. No markdown, no explanation."
            },
            {
              "role": "user",
              "content": "List 10 major public holidays in India for the year $year (including Diwali, Holi, Eid, Independence Day, etc. with accurate dates for $year). Format MUST be exactly: [{\"date\": \"YYYY-MM-DD\", \"name\": \"Holiday Name\"}]"
            }
          ],
          "temperature": 0.0
        }),
  ...
  Widget _buildCalendarTab() {
    return SingleChildScrollView(
      ...
      child: Column(children: [
            ...
            children: [
              ...
              InkWell(
                ...
                child: Row(
                  ...
                  children: [
                    const Icon(Icons.calendar_month_rounded, size: 18, color: HRTheme.primaryDark),
                    const SizedBox(width: 8),
                    Text(
                      '${["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"][_calendarDate.month - 1]} ${_calendarDate.year}',
                      style: GoogleFonts.poppins(fontSize: 15, fontWeight: FontWeight.bold, color: HRTheme.textPrimary),
                    ),
                    const Icon(Icons.arrow_drop_down, size: 18, color: HRTheme.textSecondary),
                  ],
                ),
              ),
              IconButton(
        ...
        // AI Smart Insights
        const HRSectionHeader(title: 'AI Smart Insights', icon: Icons.auto_awesome),
        HRCard(
          ...
          child: Padding(
            ...
            child: Row(
              ...
              children: [
                const Icon(Icons.tips_and_updates_outlined, color: Colors.amber, size: 24),
                  ...
                  child: _isLoadingInsight 
                    ? Row(
                        children: [
                          ...
                          Text(
                            _aiInsightText,
                            style: GoogleFonts.poppins(fontSize: 13, color: HRTheme.textSecondary, fontStyle: FontStyle.italic),
                          ),
                        ],
                      )
                    : Text(
                        _aiInsightText,
                        style: GoogleFonts.poppins(fontSize: 13, color: HRTheme.textPrimary, height: 1.4),
                      ),
                ),
              ],
            ),
        ...
        if (_selectedDay != null) ...[
          HRSectionHeader(
            title: 'Events on ${["Mon","Tue","Wed","Thu","Fri","Sat","Sun"][_selectedDay!.weekday - 1]}, ${_selectedDay!.day}/${_selectedDay!.month}/${_selectedDay!.year}',
          ...
          const Center(
            child: Padding(
              ...
              child: Text('Tap a date on the calendar to view events.', style: TextStyle(color: Colors.grey)),
            ),
          ),
        ],
      ]),
  ...
  Future<void> _fetchAiInsight() async {
    ...
    try {
      final response = await http.post(
        ...
        body: jsonEncode({
          ...
          "messages": [
            {
              "role": "system",
              "content": "You are a helpful and positive HR assistant. Keep responses under 2 short sentences. Be encouraging and friendly."
            },
            {
              "role": "user",
              "content": "The user has taken $approvedThisMonth approved leaves in the month of ${_calendarDate.month}/${_calendarDate.year}. Give a short, encouraging message about their work-life balance."
            }
          ],
          "max_tokens": 60,
          "temperature": 0.7
      ...
      if (response.statusCode == 200) {
        ...
        if (mounted) {
          setState(() {
            _aiInsightText = data['choices'][0]['message']['content'].toString().trim();
  ...
  List<Widget> _buildSelectedDayEvents() {
    ...
    if (_selectedDay != null) {
      final String searchDate = "${_selectedDay!.year}-${_selectedDay!.month.toString().padLeft(2, '0')}-${_selectedDay!.day.toString().padLeft(2, '0')}";
      final holidaysOnDay = _publicHolidays.where((h) => h['date'] == searchDate).toList();
      
      for (var holiday in holidaysOnDay) {
        eventWidgets.add(HRCard(
          ...
          child: Row(children: [
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(holiday['name'] ?? holiday['localName'] ?? 'Holiday', style: GoogleFonts.poppins(fontSize: 13, fontWeight: FontWeight.w600)),
              Text('Public Holiday', style: GoogleFonts.poppins(fontSize: 11, color: Colors.green)),
            ])),
          ]),
        ));
      }
    ...
    for (var a in dayLeaves) {
      eventWidgets.add(HRCard(
        ...
        child: Row(children: [
          Container(width: 44, height: 44,
            decoration: BoxDecoration(color: _getStatusColor(a.status).withOpacity(0.1),
                borderRadius: BorderRadius.circular(HRTheme.radiusSM)),
            child: Center(
          ...
          const SizedBox(width: 12),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(a.leaveType, style: GoogleFonts.poppins(fontSize: 13, fontWeight: FontWeight.w600)),
            Text('Status: ${a.status}', style: GoogleFonts.poppins(fontSize: 11, color: _getStatusColor(a.status))),
          ])),
        ]),
      ));
    }
    ...
    if (eventWidgets.isEmpty) {
      eventWidgets.add(
        const Padding(
          ...
          child: Text('No leaves or events scheduled for this date.', style: TextStyle(color: Colors.grey)),
        )
      );
    }
  ...
  Color _getStatusColor(String status) {
    switch (status.toLowerCase()) {
      case 'approved': return HRTheme.leave;
      case 'pending': return Colors.orange;
      case 'rejected':
      case 'cancelled': return Colors.red;
      default: return HRTheme.textPrimary;
    }
  }
  ...
  Widget _buildMiniCalendar(DateTime now) {
    ...
    final bool isCurrentMonth = now.year == DateTime.now().year && now.month == DateTime.now().month;
    ...
    return Column(children: [
      ...
      GridView.builder(
        ...
        itemBuilder: (_, i) {
          ...
          final String searchDate = "${now.year}-${now.month.toString().padLeft(2, '0')}-${day.toString().padLeft(2, '0')}";
          final isHoliday = _publicHolidays.any((h) => h['date'] == searchDate);
          
          final isSelected = _selectedDay != null && _selectedDay!.year == now.year && _selectedDay!.month == now.month && _selectedDay!.day == day;
          return GestureDetector(
            ...
            child: Container(
              decoration: BoxDecoration(
                color: isSelected ? HRTheme.primaryDark 
                    : isToday ? HRTheme.primaryDark.withOpacity(0.5)
                    : isLeave ? HRTheme.leave.withOpacity(0.15)
                    : isHoliday ? Colors.green.withOpacity(0.15)
                    : Colors.transparent,
                shape: BoxShape.circle,
                border: isSelected ? Border.all(color: HRTheme.primaryDark, width: 2) : null,
              ),
              child: Center(child: Text('$day', style: GoogleFonts.poppins(
                fontSize: 12, fontWeight: (isToday || isSelected) ? FontWeight.w800 : FontWeight.w400,
                color: (isToday || isSelected) ? Colors.white
                    : isLeave ? HRTheme.leave
                    : isHoliday ? Colors.green
                    : HRTheme.textPrimary,
              ))),
            ),
          );
  ...
  void _showDetailsDialog(Map<String, dynamic> data, LeaveApplication app) {
    ...
    final isCancelled = app.status.toLowerCase() == 'cancelled' || app.status.toLowerCase() == 'rejected';
    
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        backgroundColor: isDark ? HRTheme.bgCardDark : Colors.white,
        title: Text('Leave Details', style: GoogleFonts.poppins(fontWeight: FontWeight.bold)),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            ...
            Text('Duration: ${app.fromDate} to ${app.toDate} (${app.days} Days)'),
  ...
  Future<void> _deleteLeaveRequest(String id) async {
    try {
      ...
      _fetchData();
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Leave request deleted')));
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Error: $e')));
    }
  }
                    ...
                    children: [
                      ...
                      GestureDetector(
                        ...
                        child: Container(
                          ...
                          child: Row(
                            children: [
                              const Icon(Icons.calendar_today, size: 14, color: HRTheme.leave),
                              const SizedBox(width: 6),
                              Expanded(
                                child: Text(
                                  appDate == null ? 'Application Date *' : 'Application Date: ${appDate!.day}-${appDate!.month.toString().padLeft(2, '0')}-${appDate!.year}',
                                  style: GoogleFonts.poppins(
                                    fontSize: 13,
                                    color: appDate == null ? HRTheme.textHint : (isDark ? Colors.white : HRTheme.textPrimary),
                                  ),
                                ),
                              ),
                            ],
                        ...
                        child: Container(
                          ...
                          child: Row(
                            children: [
                              ...
                              Expanded(
                                child: Text(
                                  from == null ? 'From Date *' : 'From Date: ${from!.day}/${from!.month}/${from!.year}',
                                  style: GoogleFonts.poppins(
                                    fontSize: 13,
                                    color: from == null ? HRTheme.textHint : (isDark ? Colors.white : HRTheme.textPrimary),
                                  ),
                                ),
                              ),
                            ],
                      ...
                      DropdownButtonFormField<String>(
                        ...
                        dropdownColor: isDark ? HRTheme.bgCardDark : Colors.white,
                        items: const [
                          DropdownMenuItem(value: 'FULL_DAY', child: Text('Full Day', style: TextStyle(fontSize: 13))),
                          DropdownMenuItem(value: 'FIRST_HALF', child: Text('First Half', style: TextStyle(fontSize: 13))),
                          DropdownMenuItem(value: 'SECOND_HALF', child: Text('Second Half', style: TextStyle(fontSize: 13))),
                        ],
                        onChanged: (v) => ss(() => fromDuration = v ?? 'FULL_DAY'),
                      ),
                      const SizedBox(height: 14),
                        ...
                        child: Container(
                          ...
                          child: Row(
                            children: [
                              ...
                              Expanded(
                                child: Text(
                                  to == null ? 'To Date *' : 'To Date: ${to!.day}/${to!.month}/${to!.year}',
                                  style: GoogleFonts.poppins(
                                    fontSize: 13,
                                    color: to == null ? HRTheme.textHint : (isDark ? Colors.white : HRTheme.textPrimary),
                                  ),
                                ),
                              ),
                            ],
                      ...
                      DropdownButtonFormField<String>(
                        ...
                        onChanged: (v) => ss(() => toDuration = v ?? 'FULL_DAY'),
                      ),
                      const SizedBox(height: 14),
                      ...
                      TextFormField(
                        ...
                        key: ValueKey('totalDays_$totalDays'), // Forces rebuild when value changes
                        enabled: false,
                      ...
                      Row(
                        ...
                        children: [
                          ...
                          ElevatedButton(
                            ...
                            onPressed: () async {
                              if (selectedBalance == null || from == null || to == null || reasonCtrl.text.trim().isEmpty) {
                              ...
                              try {
                                final empId = await HRApiService.getLoggedEmpId();
                                final formattedFrom = '${from!.year}-${from!.month.toString().padLeft(2, '0')}-${from!.day.toString().padLeft(2, '0')}';
                                final formattedTo = '${to!.year}-${to!.month.toString().padLeft(2, '0')}-${to!.day.toString().padLeft(2, '0')}';
                                final appDateStr = '${appDate!.year}-${appDate!.month.toString().padLeft(2, '0')}-${appDate!.day.toString().padLeft(2, '0')}';
                                
                                // Format payload matching web app precisely:
                                final payload = {
                                  "applicationDate": appDateStr,
                                  "fromDate": formattedFrom,
                                  "fromDateStr": formattedFrom,
                                  "toDate": formattedTo,
                                  "toDateStr": formattedTo,
                                  "fromDateDuration": fromDuration,
                                  "toDateDuration": toDuration,
                                  "leaveNameId": selectedBalance!.leaveNameId ?? 0,
                                  "leaveName": selectedBalance!.leaveType,
