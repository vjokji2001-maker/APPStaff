...
class ApiService {
  static const String _defaultTokenPrefix = 'SmartCare ';
  ...
  static Future<Map<String, dynamic>?> loginUser(
    ...
    try {
      ...
      if (response.statusCode == 409 && _isActiveSessionConflict(response.body)) {
        ...
        debugPrint('Forced login status: ${forcedResponse.statusCode}');
        debugPrint('Forced login body: ${forcedResponse.body}');

        return await _handleLoginResponse(forcedResponse);
      }
  ...
  static Future<http.Response> _performLoginRequest({
  ...
  static bool _isActiveSessionConflict(String responseBody) {
    try {
      final decoded = jsonDecode(responseBody);
      if (decoded is Map<String, dynamic>) {
        final message = (decoded['message'] ?? '').toString();
        final error = decoded['error'];
        final cause = error is Map<String, dynamic>
            ? (error['cause'] ?? '').toString()
            : '';
        return message == 'ACTIVE_SESSION_EXISTS' ||
            cause.toLowerCase().contains('active session');
      }
    } catch (_) {}
    return false;
  }
  ...
  static Future<Map<String, dynamic>?> _handleLoginResponse(
    ...
    if (data != null) {
      await SessionManager.saveSession(
        bearer: _defaultTokenPrefix.trim(),
        token: data['token'] ?? '',
  ...
  static Future<void> _extractAndStoreTokens(
    ...
    debugPrint(
      'Access token: ${accessToken != null ? '${accessToken!.substring(0, min(20, accessToken!.length))}...' : 'null'}',
    );
  ...
  static void _startAutoRefreshTimer() {
    ...
    if (refreshDelay > 0) {
      ...
      debugPrint('Token expiring very soon - refreshing now');
      refreshUserToken();
    }
  }
  ...
  static Future<http.Response?> authenticatedRequest(
    ...
    try {
      ...
      if (response?.statusCode == 401) {
        debugPrint('401 - refreshing and retrying');
  ...
  static Future<http.Response?> _makeRequest(
    ...
    switch (method.toUpperCase()) {
      case 'GET':
        return ApiDebugHttp.get(uri, headers: headers);
      case 'POST':
        return ApiDebugHttp.post(uri, headers: headers, body: encoded);
      case 'PUT':
        return ApiDebugHttp.put(uri, headers: headers, body: encoded);
      case 'DELETE':
        return ApiDebugHttp.delete(uri, headers: headers);
      default:
